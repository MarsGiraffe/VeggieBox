									...
									<c:forEach var="cart" items="${cartList}">
										<tr>
											...
											<td class="qty">
												...
												<button type = "button" onclick = "delCart('${cart.cart_num}')" class="delete-button" style = "background-color: white; border:1px solid black; border-radius: 6px; width: 25px; height:25px; font-weight: bold;">X</button>
											</td>
										</tr>
	    ...
	    if (confirm("상품을 삭제하시겠습니까?")) {
	      $.ajax({
	        ...
	        success: function(data) {
	          // 삭제가 성공적으로 이루어지면, 페이지의 장바구니 목록에서 삭제된 항목을 제거
